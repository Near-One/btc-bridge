...
pub fn bytes_to_btc_transaction(tx_bytes: &[u8], chain: &network::Chain) -> WrappedTransaction {
    WrappedTransaction::decode(tx_bytes, chain).expect("Deserialization tx_bytes failed")
}

#[cfg(test)]
mod tests {
    use crate::network::Chain::ZcashMainnet;
    use crate::network::{Address, Chain};
    use crate::{bytes_to_btc_transaction, get_deposit_path, network, DepositMsg};
    ...
    fn test_zcash_tx_bytes() {
        ...
        let btc_tx = bytes_to_btc_transaction(&tx_zcash_bytes, &network::Chain::ZcashMainnet);
