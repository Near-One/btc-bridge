mod setup;
use bitcoin::{CompressedPublicKey, Transaction as BtcTransaction};
...
/// Known compressed public key for sender address tests.
const SENDER_PUBKEY_HEX: &str =
    "0279BE667EF9DCBBAC55A06295CE870B07029BFCDB2DCE28D959F2815B16F81798";

/// Derive the P2WPKH mainnet address for the test pubkey.
fn sender_p2wpkh_address() -> String {
    let pubkey_bytes = hex::decode(SENDER_PUBKEY_HEX).unwrap();
    let pubkey = CompressedPublicKey::from_slice(&pubkey_bytes).unwrap();
    let wp = bitcoin::WitnessProgram::p2wpkh(&pubkey);
    bitcoin::Address::from_witness_program(wp, bitcoin::Network::Bitcoin).to_string()
}

/// Derive the P2PKH mainnet address for the test pubkey.
fn sender_p2pkh_address() -> String {
    let pubkey_bytes = hex::decode(SENDER_PUBKEY_HEX).unwrap();
    let pubkey = bitcoin::PublicKey::from_slice(&pubkey_bytes).unwrap();
    bitcoin::Address::p2pkh(pubkey, bitcoin::Network::Bitcoin).to_string()
}

#[tokio::test]
#[cfg(not(feature = "zcash"))]
async fn test_refund_without_refund_address_in_deposit_msg() {
    let worker = near_workspaces::sandbox().await.unwrap();
    let context = Context::new(&worker, Some(CHAIN.to_string())).await;

    let sender_address = sender_p2wpkh_address();

    // deposit_msg WITHOUT refund_address
    let deposit_msg = DepositMsg {
        recipient_id: context.get_account_by_name("alice").sdk_id(),
        post_actions: None,
        extra_msg: None,
    ...
    // Build tx with P2WPKH witness — sender is identifiable from inputs
    let tx_bytes = generate_transaction_bytes_with_witness(
        vec![(
            "c1c1069f02ad4ca31a16113903ab9fe9e8da6ddf20cad4b461b71e8b96050f80",
            0,
        )],
        vec![(deposit_address.as_str(), 100_000)],
        SENDER_PUBKEY_HEX,
    ...
    // request_refund with sender's address (derived from input witness) — should succeed
    check!(
        print "request_refund (sender address from witness)"
        context.request_refund(
            "alice",
            deposit_msg.clone(),
            &sender_address,
...
#[tokio::test]
#[cfg(not(feature = "zcash"))]
async fn test_refund_without_refund_address_wrong_sender() {
    let worker = near_workspaces::sandbox().await.unwrap();
    let context = Context::new(&worker, Some(CHAIN.to_string())).await;
    ...
    // Build tx with P2WPKH witness from SENDER_PUBKEY
    let tx_bytes = generate_transaction_bytes_with_witness(
        vec![(
            "d2d2069f02ad4ca31a16113903ab9fe9e8da6ddf20cad4b461b71e8b96050f90",
            0,
        )],
        vec![(deposit_address.as_str(), 100_000)],
        SENDER_PUBKEY_HEX,
    ...
    // request_refund with a DIFFERENT address — should fail
    check!(
        context.request_refund(
            "alice",
            deposit_msg,
...
#[tokio::test]
#[cfg(not(feature = "zcash"))]
async fn test_refund_p2pkh_input_without_refund_address() {
    let worker = near_workspaces::sandbox().await.unwrap();
    let context = Context::new(&worker, Some(CHAIN.to_string())).await;

    let sender_address = sender_p2pkh_address();
    ...
    // Build tx with P2PKH script_sig — sender is identifiable from script_sig
    let tx_bytes = generate_transaction_bytes_with_p2pkh_scriptsig(
        vec![(
            "e3e3069f02ad4ca31a16113903ab9fe9e8da6ddf20cad4b461b71e8b96050fa0",
            0,
        )],
        vec![(deposit_address.as_str(), 100_000)],
        SENDER_PUBKEY_HEX,
    ...
    // request_refund with sender's P2PKH address — should succeed
    check!(
        print "request_refund (P2PKH sender)"
        context.request_refund(
            "alice",
            deposit_msg,
            &sender_address,
...
#[tokio::test]
#[cfg(not(feature = "zcash"))]
async fn test_refund_p2pkh_input_wrong_sender() {
    let worker = near_workspaces::sandbox().await.unwrap();
    let context = Context::new(&worker, Some(CHAIN.to_string())).await;
    ...
    // Build tx with P2PKH script_sig
    let tx_bytes = generate_transaction_bytes_with_p2pkh_scriptsig(
        vec![(
            "f4f4069f02ad4ca31a16113903ab9fe9e8da6ddf20cad4b461b71e8b96050fb0",
            0,
        )],
        vec![(deposit_address.as_str(), 100_000)],
        SENDER_PUBKEY_HEX,
    ...
    // request_refund with a DIFFERENT P2PKH address — should fail
    check!(
        context.request_refund(
            "alice",
            deposit_msg,
