# Refund Flow — verify_deposit after refund fails

User deposits BTC, requests and executes refund. Later, Relayer tries
to call `verify_deposit` for the same UTXO — it fails because the UTXO
was already spent on Bitcoin by the refund transaction.
...
| 9 | `verify_deposit` | `contracts/satoshi-bridge/src/api/bridge.rs:22` |
